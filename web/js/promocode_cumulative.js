...
usesgraphcrt.promocodeCumulative = {
    ...
    init: function(){
        ...
        $(document).on('click','[data-role=remove-row]',function () {
            ...
            var data = {
                'promocodeId' : usesgraphcrt.promocodeCumulative.getUrlVar()["id"],
                'conditionId' : $(this).data('condition')
            };
            usesgraphcrt.promocodeCumulative.deleteCondition(delete_url,data,block);
            $(currentBlock).remove();
        });
    ...
    addRow: function(e) {
        e.preventDefault();
        usesgraphcrt.promocodeCumulative.cumulativeRowIncrement++;
        $('.cumulative-block').append($('<div class="cumulative-row form-group"></div>')
            .append('<input class="form-control" name="Conditions[C'+usesgraphcrt.promocodeCumulative.cumulativeRowIncrement+'][sumStart]" type="text" placeholder="От"> - ')
            .append(' <input class="form-control" name="Conditions[C'+usesgraphcrt.promocodeCumulative.cumulativeRowIncrement+'][sumStop]" type="text"placeholder="До">')
            .append(' <input class="form-control" name="Conditions[C'+usesgraphcrt.promocodeCumulative.cumulativeRowIncrement+'][percent]" type="text" style="width: 50px"placeholder="%">')
...
usesgraphcrt.promocodeCumulative.init();
usesgraphcrt.promocodeCumulative.onLoad();